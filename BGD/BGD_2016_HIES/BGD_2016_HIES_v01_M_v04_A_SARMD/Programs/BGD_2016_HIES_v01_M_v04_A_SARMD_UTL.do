/*------------------------------------------------------------------------------
					GMD Harmonization
--------------------------------------------------------------------------------
<_Program name_>   BGD_2016_HIES_v01_M_v01_A_GMD_UTL.do	</_Program name_>
...
<_Version Control_>
Date:	03-03-2019
File:	BGD_2016_HIES_v01_M_v01_A_GMD_UTL.do
- First version
</_Version Control_>
------------------------------------------------------------------------------*/
...
*<_Raw data_>
tempfile raw1
datalibweb, country(BGD) year(2016) type(SARRAW) surveyid(BGD_2016_HIES_v01_M) filename(HH_SEC_9D2.dta)
keep hhid s9d2q00 s9d2q01
drop if s9d2q00 == .
keep if inlist(s9d2q00,489,393,395,396,397)
reshape wide s9d2q01, i(hhid) j(s9d2q00)
save `raw1'

tempfile raw2
datalibweb, country(BGD) year(2016) type(SARRAW) surveyid(BGD_2016_HIES_v01_M) filename(HH_SEC_9C.dta)
keep hhid s9cq00 s9cq03
keep if inlist(s9cq00, 248, 296, 293, 247, 269, 292, 244, 242, 243, 245, 271, 294, 241, 246)
reshape wide s9cq03, i(hhid) j(s9cq00)

merge 1:1 hhid using `raw1'
drop _merge
save `raw2'

tempfile raw3
datalibweb, country(BGD) year(2016) type(SARRAW) surveyid(BGD_2016_HIES_v01_M) filename(HH_SEC_6A.dta)
merge 1:1 hhid using `raw2'
...
gen coal_exp= 12*s9cq03248;
notes coal_exp: Originally collected monthly, multiplied by 12 to annualize;
*</_coal_exp_>;
...
gen comm_exp= 12*s9cq03296;
notes comm_exp: created directly from survey, doesn't include telefax. Multiplied by 12 to annualize;
*</_comm_exp_>;

*<_cooksource_>;
*<_cooksource_note_> Main cooking fuel *</_cooksource_note_>;
*<_cooksource_note_> cooksource brought in from  *</_cooksource_note_>;
gen cooksource= s6aq06;
recode cooksource (1=4) (2=5) (3/6 = 9) (7=10);
*</_cooksource_>;
...
gen diesel_exp = 12*s9cq03293;
notes diesel_exp: Multiplied by 12 to annualize;
*</_diesel_exp _>;
...
egen dwelsvc_exp= rowtotal(s9d2q01395 s9d2q01396 s9d2q01397);
*</_dwelsvc_exp_>;
...
gen garbage_exp= 12*s9cq03269;
notes garbage_exp: multiplied by 12 to annualize;
*</_garbage_exp_>;
...
gen gasoline_exp = 12*s9cq03292;
notes gasoline_exp: Multiplied by 12 to annualize;
*</_gasoline_exp _>;
...
egen othsol_exp= rowtotal(s9cq03242 s9cq03243 s9cq03245), missing ;
replace othsol_exp = 12*othsol_exp;
...
replace sanitation_original = "1-Sanitary" if sanitation_original == "Sanitary";
replace sanitation_original = "2-Pacca latrine (Water seal)" if sanitation_original == "Pacca latrine (Water seal)";
replace sanitation_original = "3-Pacca latrine (Pit)" if sanitation_original == "Pacca latrine (Pit)";
replace sanitation_original = "4-Kacha latrine (perm)" if sanitation_original == "Kacha latrine (perm)";
replace sanitation_original = "5-Kacha latrine (temp)" if sanitation_original == "Kacha latrine (temp)";
replace sanitation_original = "6-Open space/no latrine" if sanitation_original == "Other";
*</_sanitation_original_>;
...
gen sanitation_source = s6aq10;
recode sanitation_source (2 3 = 4) (6 = 13) (4 5 = 14);
*</_sanitation_source_>;
...
egen transfuel_exp= rowtotal(s9cq03292 s9cq03293 s9cq03294), missing;
...
egen tvintph_exp= rowtotal(internet_exp tel_exp tv_exp);
*</_tvintph_exp_>;
...
encode water_original_sarmd,gen(temp);
label define temp 3 "1-Supply water" 4 "2-Tubewell" 2 "3-Pond/river" 6 "4-Well" 5 "5-Waterfall/string" 1 "6-Other", modify;
...
replace piped = 0 if water_source != .;
...
replace piped_to_prem =  0 if water_source != . ;
replace piped_to_prem =  0 if piped_to_prem == .;
*</_piped_to_prem_>;
