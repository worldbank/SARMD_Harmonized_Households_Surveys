/*------------------------------------------------------------------------------
					GMD Harmonization
--------------------------------------------------------------------------------
<_Program name_>   		BGD_2022_HIES_v02_M.do				   </_Program name_>
...
merge m:1 PSU HHID using "${input}\weight_final_2022.dta", keepusing (domain16 hh_wgt pop_wgt urbrural stratum16 rmo)
drop if _merge==2
...
****** 2 - COUNT NUMBER OF OBS WITHIUT MISSINGS AND ZEROS BY STRATUM, URBAN AND RURAL, AND INDUSTRY 		
foreach var in S4BQ02C S4BQ05B S4BQ08 S4BQ09 {
...
* Save info for those households with income info but without employment info
preserve
...
keep  PSU HHID PID agri_net agri_income agri_expend crop_cons crop_sold livestock_cons livestock_sold byproduct_cons byproduct_sold fish_cons fish_sold tree_cons tree_sold share_ind
order PSU HHID PID agri_net agri_income agri_expend crop_cons crop_sold livestock_cons livestock_sold byproduct_cons byproduct_sold fish_cons fish_sold tree_cons tree_sold share_ind
sort  PSU HHID PID
gen   x2=1
save `section7_2'	
...
keep  PSU HHID PID w_cat 	hours months S4AQ01A S4AQ01B S4BQ06 daylab_cash daylab_kind employee_cash employee_kind month_nonagri agri_net act anomalies share_ind
order PSU HHID PID w_cat 	hours months S4AQ01A S4AQ01B S4BQ06 daylab_cash daylab_kind employee_cash employee_kind month_nonagri agri_net act anomalies share_ind
reshape wide       w_cat 	hours months S4AQ01A S4AQ01B S4BQ06 daylab_cash daylab_kind employee_cash employee_kind month_nonagri agri_net anomalies share_ind, j(act) i(PSU HHI PID)
...
* OLD-AGE ALLOWANCE
replace S1CQ10A = 1500			if  S1CQ02==1 &    S1CQ10A>0 & S1CQ10A<1500
replace S1CQ10A = 1500			if  S1CQ02==1 & S1CQ10A>1500 & S1CQ10A<2250
replace S1CQ10A = 3000			if  S1CQ02==1 & S1CQ10A>2250 & S1CQ10A<3000
replace S1CQ10A = 3000			if  S1CQ02==1 & S1CQ10A>3000 & S1CQ10A<3750
replace S1CQ10A = 4500			if  S1CQ02==1 & S1CQ10A>3750 & S1CQ10A<4500
replace S1CQ10A = 4500			if  S1CQ02==1 & S1CQ10A>4500 & S1CQ10A<5250
replace S1CQ10A = 6000			if  S1CQ02==1 & S1CQ10A>5250 & S1CQ10A<6000
replace S1CQ10A = 6000			if  S1CQ02==1 & S1CQ10A>6000 & S1CQ10A<.

* WIDOWS ALLOWANCE
replace S1CQ10A = 1500			if  S1CQ02==2 &    S1CQ10A>0 & S1CQ10A<1500
replace S1CQ10A = 1500			if  S1CQ02==2 & S1CQ10A>1500 & S1CQ10A<2250
replace S1CQ10A = 3000			if  S1CQ02==2 & S1CQ10A>2250 & S1CQ10A<3000
replace S1CQ10A = 3000			if  S1CQ02==2 & S1CQ10A>3000 & S1CQ10A<3750
replace S1CQ10A = 4500			if  S1CQ02==2 & S1CQ10A>3750 & S1CQ10A<4500
replace S1CQ10A = 4500			if  S1CQ02==2 & S1CQ10A>4500 & S1CQ10A<5250
replace S1CQ10A = 6000			if  S1CQ02==2 & S1CQ10A>5250 & S1CQ10A<6000
replace S1CQ10A = 6000			if  S1CQ02==2 & S1CQ10A>6000 & S1CQ10A<.

* DISABLED ALLOWANCE
replace S1CQ10A = 2250			if  S1CQ02==3 &    S1CQ10A>0 & S1CQ10A<2250
replace S1CQ10A = 2250			if  S1CQ02==3 & S1CQ10A>2250 & S1CQ10A<3375
replace S1CQ10A = 4500			if  S1CQ02==3 & S1CQ10A>3375 & S1CQ10A<4500
replace S1CQ10A = 4500			if  S1CQ02==3 & S1CQ10A>4500 & S1CQ10A<5625
replace S1CQ10A = 6750			if  S1CQ02==3 & S1CQ10A>5625 & S1CQ10A<6750
replace S1CQ10A = 6750			if  S1CQ02==3 & S1CQ10A>6750 & S1CQ10A<7875
replace S1CQ10A = 9000			if  S1CQ02==3 & S1CQ10A>7875 & S1CQ10A<9000
replace S1CQ10A = 9000			if  S1CQ02==3 & S1CQ10A>9000 & S1CQ10A<.
...
collapse (sum) ssn_cash ssn_kind, by(PSU HHID PID S1CQ02)
egen ssn = rsum(ssn_cash ssn_kind), missing
reshape wide ssn_cash ssn_kind ssn, i(PSU HHID PID) j(S1CQ02)
sort  PSU HHID PID
save `sect1c', replace

use `sect_4_5_7_8_9'
merge 1:1 PSU HHID PID using `sect1c'
drop _merge
sort PSU HHID PID
...
tempfile stipend
keep if S2BQ04>=1 & S2BQ04<=3
keep if S2BQ06>0
gen PID = S2BQ00

gen  stipend_inc = S2BQ06/12		
keep PSU HHID PID stipend_inc
sort PSU HHID PID
save `stipend'
...
**** Section 9A1 - DAILY CONSUMED FOOD ITEMS/COMMODITIES
use "${input}\HH_SEC_9A1.dta", clear 
tempfile daily
merge m:1 PSU HHID using "${input}\weight_final_2022.dta", keepusing (domain16 hh_wgt)
drop if _merge==2

* Median by domain
gen median = .
levelsof domain16, local(psu)
levelsof S9A2Q01 if S9A2Q09==2, local(good)
foreach s of local psu {
	foreach t of local good {
	    di in red "`s'"
		di in red "`t'"
		sum S9A2Q07 [aw=hh_wgt] 	if  domain16==`s' & S9A2Q01==`t' & S9A2Q09==1, detail
		replace median = r(p50) 	if  domain16==`s' & S9A2Q01==`t' & S9A2Q09==2 & median==.
		replace median = r(p75) 	if  domain16==`s' & S9A2Q01==`t' & S9A2Q09==2 & median<=S9A2Q07
		replace median = r(p90) 	if  domain16==`s' & S9A2Q01==`t' & S9A2Q09==2 & median<=S9A2Q07
		replace median = r(p95) 	if  domain16==`s' & S9A2Q01==`t' & S9A2Q09==2 & median<=S9A2Q07
		replace median = r(p99) 	if  domain16==`s' & S9A2Q01==`t' & S9A2Q09==2 & median<=S9A2Q07
		}
	}
* MAJOR SOURCE 
*  2 = OMS
*  5 = Provided by the government
keep if S9A2Q09==2 | S9A2Q09==5

gen 	S9A2Q08_adjust = S9A2Q08					if  S9A2Q09==5
replace S9A2Q08_adjust = 0							if  S9A2Q09==2 & median<=S9A2Q07
replace S9A2Q08_adjust = S9A2Q05*(median-S9A2Q07)	if  S9A2Q09==2 & median>S9A2Q07

collapse (sum) S9A2Q08 S9A2Q08_adjust, by(PSU HHID)
replace S9A2Q08 = S9A2Q08*2.17				/* information for each household is captured over a period of 14 days */
replace S9A2Q08_adjust = S9A2Q08_adjust*2.17
sort PSU HHID
save `daily'	
	
**** Section 9A2 - WEEKLY CONSUMED FOOD
use "${input}\HH_SEC_9B2.dta", clear 
tempfile weekly
merge m:1 PSU HHID using "${input}\weight_final_2022.dta", keepusing (domain16 urb hh_wgt)
drop if _merge==2

* Median by urban-rural
gen median = .
levelsof urbrur, local(psu)
levelsof SERIAL if S9B2Q05==2, local(good)
foreach s of local psu {
	foreach t of local good {
		sum S9B2Q03U [aw=hh_wgt] 	if  urbrur==`s' & SERIAL==`t' & S9B2Q05==1, detail
		replace median = r(p50) 	if  urbrur==`s' & SERIAL==`t' & S9B2Q05==2 & median==.
		replace median = r(p75) 	if  urbrur==`s' & SERIAL==`t' & S9B2Q05==2 & median<=S9B2Q03U
		replace median = r(p90) 	if  urbrur==`s' & SERIAL==`t' & S9B2Q05==2 & median<=S9B2Q03U
		replace median = r(p95) 	if  urbrur==`s' & SERIAL==`t' & S9B2Q05==2 & median<=S9B2Q03U
		replace median = r(p99) 	if  urbrur==`s' & SERIAL==`t' & S9B2Q05==2 & median<=S9B2Q03U
		}
	}
* MAJOR SOURCE 
*  2 = OMS
*  5 = Provided by the government
keep if S9B2Q05==2 | S9B2Q05==5

gen 	S9B2Q04_adjust = S9B2Q04					if  S9B2Q05==5
replace S9B2Q04_adjust = 0							if  S9B2Q05==2 & median<=S9B2Q03U
replace S9B2Q04_adjust = S9B2Q02*(median-S9B2Q03U)	if  S9B2Q05==2 & median>S9B2Q03U
collapse (sum) S9B2Q04 S9B2Q04_adjust, by(PSU HHID)

replace S9B2Q04 = S9B2Q04*2.17				/* information for each household is captured over a period of 2 weeks */
replace S9B2Q04_adjust = S9B2Q04_adjust*2.17
sort PSU HHID
save `weekly'
...
foreach j in hh_roster housing assets assets_animal land expenditures1 expenditures2 daily weekly {
...
gen     ssn20_alt = .
replace ssn20_alt = stipend_inc			if  stipend_inc>0 & stipend_inc<. & educat>=1 & educat<=2
replace ssn20_alt = ssn20				if  ssn20_alt==. & ssn20>0 & ssn20<.

gen     ssn21_alt = .
replace ssn21_alt = stipend_inc			if  stipend_inc>0 & stipend_inc<. & educat==4
replace ssn21_alt = ssn21				if  ssn21_alt==. & ssn21>0 & ssn21<.

gen     ssn22_alt = .
replace ssn22_alt = stipend_inc			if  stipend_inc>0 & stipend_inc<. & educat>=6 & educat<=7
replace ssn22_alt = ssn22				if  ssn22_alt==. & ssn22>0 & ssn22<.
drop educat atschool
