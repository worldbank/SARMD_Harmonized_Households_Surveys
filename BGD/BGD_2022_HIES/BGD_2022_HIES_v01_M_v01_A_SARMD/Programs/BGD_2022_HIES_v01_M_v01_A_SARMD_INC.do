...
local filename     	"BGD_2022_HIES_v01_M_v01_A_SARMD_INC"
*</_Program setup_>

*<_Folder creation_>
*cap mkdir "$rootdatalib\\BGD\BGD_2022_HIES\BGD_2022_HIES_v01_M_v01_A_SARMD\Data"
*cap mkdir "$rootdatalib\\BGD\BGD_2022_HIES\BGD_2022_HIES_v01_M_v01_A_SARMD\Data\Harmonized\"
*</_Folder creation_>
...
* INDIVIDUAL ROSTER
tempfile roster 
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_1A1.dta) clear  //change
...
tempfile sect4
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_4A.dta) clear  //change
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_05.dta) clear  //change
* Duplicated business, with missing info in S5Q00 (activity numbeR) //change 
...
*datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_4A.dta) clear  //change
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_05.dta) //change to this one????
sort      PSU HHID
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_7B.dta) clear  //change
keep 	if  S7BQ02==1
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_7C1.dta) clear  //change
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_7C2.dta) clear  //change
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_7C3.dta) clear  //change
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_7C4.dta) clear  //change
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_7D.dta) clear  //change
keep	if  S7DQ02==1
...
* Worked Hours (Year)
rename  hours aux_hours
gen     hours = S4AQ02 * S4AQ03 * S4AQ04
replace hours = aux_hours		if  hours==. & aux_hours!=.
drop aux_hours
...
local var "w_cat hours S4AQ01A S4AQ01B S4BQ06 daylab_cash daylab_kind employee_cash employee_kind month_nonagri agri_net anomalies"
foreach v in `var' {
	rename `v' `v'_
	}
...
keep  PSU HHID PID w_cat hours S4AQ01A S4AQ01B S4BQ06 daylab_cash daylab_kind employee_cash employee_kind month_nonagri agri_net act anomalies
order PSU HHID PID w_cat hours S4AQ01A S4AQ01B S4BQ06 daylab_cash daylab_kind employee_cash employee_kind month_nonagri agri_net act anomalies
reshape wide      w_cat hours S4AQ01A S4AQ01B S4BQ06 daylab_cash daylab_kind employee_cash employee_kind month_nonagri agri_net anomalies, j(act) i(PSU HHI PID)
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_8B.dta) clear  //change
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_9D1.dta) clear  //change
...
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(HH_SEC_1C.dta) clear //change
*gen   PID = S1CQ00D //change. This is an error. 
gen   PID = S1CQ00 //Proposal 
order PSU HHID PID
...
*use "weight_final_2022.dta" //change
datalibweb, country(BGD) year(2022) type(SARRAW) surveyid(BGD_2022_HIES_v01_M) filename(weight_final_2022.dta) clear //proposal
sort PSU HHID
tempfile weight
save `weight'
...
*<_Save data file_>
do   "$rootdatalib\\`code'\\`yearfolder'\\`SARMDfolder'\Programs\SecondOrder_INC.do"
