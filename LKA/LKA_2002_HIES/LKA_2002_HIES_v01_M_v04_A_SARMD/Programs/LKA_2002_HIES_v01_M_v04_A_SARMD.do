...
** MODFIED BY		Adriana Castillo Castillo
** Date				03/11/2021
**                                                                                                  **
******************************************************************************************************
*****************************************************************************************************/
...
** DIRECTORY
	...
	*/
	...
	glo input     "$rootdatalibweb\SAR_DATABANK\LKA\LKA_2002_HIES\LKA_2002_HIES_v01_M"
	glo output    "$rootdatalibweb\SAR_DATABANK\LKA\LKA_2002_HIES\LKA_2002_HIES_v01_M_v04_A_SARMD"
	glo pricedata "$rootdatalibweb\CPI\cpi_ppp_sarmd_weighted.dta"
	glo shares    "$rootdatalibweb\APPS\DATA CHECK\Food and non-food shares\LKA"
	glo fixlabels "$rootdatalibweb\APPS\DATA CHECK\Label fixing"
	
** LOG FILE
	log using "$output\Doc\Technical\LKA_2002_HIES_v01_M_v04_A_SARMD.log",replace
...
*<_spdef_>
	...
	*la var spdef "Spatial deflator"
	gen spdef=cpi_shiva
	notes: The neww deflactor of LKA 2002 is the one called "cpi_shiva". This change was made on March 3rd, 2021. The deflactor that was used before was spdef, that was defined by rpccons/npccons. However, the difference between both of them is just decimals. 
*</_spdef_>
...
*<_cpi_>
	...
	*keepusing(countrycode year urb syear cpi`year'_w ppp`year')
	...
	use "$pricedata", clear 
	tempfile pricedata 
	keep if countrycode=="LKA" & year==2002
	local year=2011
...
** KEEP VARIABLES - ALL
	...
	keep countrycode year survey idh idp wgt pop_wgt strata psu vermast veralt urban sector int_month /// 
	int_year subnatid1 subnatid2 subnatid3 gaul_adm1_code gaul_adm2_code ownhouse landholding tenure  ///
	water  electricity toilet landphone cellphone computer internet hsize relationharm relationcs     ///
	male age soc marital ed_mod_age everattend atschool electricity literacy educy educat4 educat5    ///
	educat7 lb_mod_age lstatus lstatus_year empstat empstat_year njobs njobs_year ocusec nlfreason    ///
	unempldur_l unempldur_u industry_orig industry occup_orig occup firmsize_l firmsize_u whours wage ///
	unitwage empstat_2 empstat_2_year industry_2 industry_orig_2 occup_2 wage_2 unitwage_2 contract   ///
	healthins socialsec union rbirth_juris rbirth rprevious_juris rprevious yrmove landphone          ///
	cellphone computer radio television fan sewingmachine washingmachine refrigerator lamp bicycle    ///
	motorcycle motorcar cow buffalo chicken pline_nat pline_int poor_nat poor_int spdef cpi ppp       ///
	cpiperiod welfare welfshprosperity welfarenom welfaredef welfarenat food_share nfood_share        ///
	quintile_cons_aggregate decile_cons_aggregate welfareother welfaretype welfareothertype  hhid
...
** DELETE MISSING VARIABLES
	...
	keep countrycode year survey idh idp wgt pop_wgt strata psu vermast veralt subnatid1 subnatid2 `keep' atschool hhid *type
	compress

	saveold "$output\Data\Harmonized\LKA_2002_HIES_v01_M_v04_A_SARMD_ALL.dta", replace version(12)
	*saveold "D:\SOUTH ASIA MICRO DATABASE\SAR_DATABANK\__REGIONAL\Individual Files\LKA_2002_HIES_v01_M_v04_A_SARMD-FULL_IND.dta", replace version(12)
	notes
	log close
...
note _dta: "LKA 2002" Variables NAMED with "v2" are those not compatible with latest round (2012). ///
 These include the existing information from the particular survey, but the iformation should be used for comparability purposes  
	...
	saveold "$output\Data\Harmonized\LKA_2002_HIES_v01_M_v04_A_SARMD_IND.dta", replace version(12)
	*saveold "D:\SOUTH ASIA MICRO DATABASE\SAR_DATABANK\__REGIONAL\Individual Files\LKA_2002_HIES_v01_M_v04_A_SARMD_IND.dta", replace version(12)
