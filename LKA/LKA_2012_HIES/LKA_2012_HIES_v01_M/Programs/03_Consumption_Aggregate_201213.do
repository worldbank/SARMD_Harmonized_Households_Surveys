...
* Author: Lidia Ceriani (lceriani@worldbank.org)
...
global out	"${data}\poverty"

* The poverty line has been computed in 2002: 1423 Rs
* It has been then monthly updated using the Colombo Consumer Price Index
* The base has been changed in 2011 to be 2006/07
* In 2006/07 prices, the poverty line is 2142 
*(http://www.statistics.gov.lk/poverty/monthly_poverty/index.htm)
* The average ccpi between July 2012 and June 2013 is 169.20833
*(http://www.statistics.gov.lk/price/ccpi(new)/Movements%20of%20CCPI(200607).pdf)
*-------------------------------------------------------------------------------
local pov_line_0607 = 2142
local ccpi_1213 = 169.20833
*-------------------------------------------------------------------------------

* Create temporary filenames for later use
*-------------------------------------------------------------------------------
tempfile 201213_sec_1_all_demographic 201213_sec_1_demographic 201213_hh_master 201213_food ///
	     201213_food_hh 201213_hh_mastercon 201213_monthly_nf 201213_hh_nfood 201213_sbexp 201213_sbexp_hh ///
	     201213_hh_pcexp 201213_f_major 201213_nf_major ashare0 pricefdb0 pricep0 priced0 pricen0 priceshare0 
...
* check that obs. are uniquely identified
*-------------------------------------------------------------------------------  
duplicates report hhid pid

save `201213_sec_1_all_demographic', replace

* drop members living outside the household
*-------------------------------------------------------------------------------
drop if person_serial_no >=40

collapse (first) district province sector psu* month weight (count) person_serial_no, by(hhid)
...
save `201213_hh_master', replace

*===============================================================================
* FOOD Expenditure
*===============================================================================
use "${data}\sec_4_1_food_exp.dta", clear
...
rename value val
rename quantity qty
rename inkind_value	kind_val

* result = 1 if questionnaire fully completed
*-------------------------------------------------------------------------------
drop if result~=1

mvencode qty val kind_val, mv(0) override
...
* LC exclude items with not recognizable code
*-------------------------------------------------------------------------------
drop if itc==440
drop if itc==140
...
lab var mfkind_trim "Monthly Trimmed Imputed Value"

mvencode mlt_val mlt_qty mlt_kind mfkind_trim, mv(0) override

keep hhid district province mfval mfquant mfkind* mlt_val mlt_qty mlt_kind itc mf_code
sort hhid
save `201213_food', replace

collapse (sum) mfval mfkind* mlt_val mlt_kind, by(hhid)
sort hhid
save `201213_food_hh', replace
...
*-------------------------------------------------------------------------------
use `201213_hh_master', clear
merge 1:1 hhid using `201213_food_hh'
assert _m == 3
drop _m

save `201213_hh_mastercon', replace

*===============================================================================
* create file with NONFOOD expenditures
*===============================================================================
use "${data}\sec_4_2_nonfood.dta", clear

* Rename some variables to use old dofile
*-------------------------------------------------------------------------------
rename nf_code itc
rename nf_value val
rename nf_inkind_value kind_val
rename nf_quantity qty

drop if result~=1

* exclude invalid items
*-------------------------------------------------------------------------------
 drop if itc==2112 | itc==2506

* Generate monthly factor for converting everything in monthly
*-------------------------------------------------------------------------------
gen 	mfact = 1 	if (itc< 3000)	|	(itc > 3400 & itc < 3500)
replace mfact = 6 	if (itc > 3000 	& itc < 3300) 
replace mfact = 12 	if (itc > 3300 	& itc < 3400)	|(itc > 3500 & itc < 3510)
lab var mfact "month factor"

mvencode val kind_val, mv(0) override

gen mnf_val 	= val / mfact
lab var mnf_val "Monthly non food value"
gen mnf_kval 	= kind_val / mfact
lab var mnf_kval "Monthly non food imputed value"
gen mnf_qty 	= qty/ mfact
lab var mnf_qty "Monthly non food quantity"

* replace itc=2501 if itc == 2510 // 2510 invalid and to be 2501

* Group items in broader categories
*-------------------------------------------------------------------------------
#delimit ; 
recode itc 	(2001/2003=1 housing)
			(2101/2119=2 fuel)
...
* Durables and Rarely occurring expenditures
* ------------------------------------------------------------------------------
drop if itc >= 3301 &   itc <= 3509

* exclude items that were included in 201213 (but not in 2002)
* ------------------------------------------------------------------------------
drop if itc == 3506 // purchased properties houses
drop if itc == 3507 // loans given

* LC
* exclude items that were included in 201213 (but not in 2009_10)
*-------------------------------------------------------------------------------
drop if itc == 2309 // Scan
drop if itc == 2310 // X-Ray

* exclude items for which recall period has changed between 2002 and 201213 
*-------------------------------------------------------------------------------
drop if itc == 3501 // 
drop if itc == 3502 // 
drop if itc == 3503 // 
...
gen 	mnf_kval_trim 	= mnf_kval if (itc <3300 & itc!=2310 & itc!=2309)
*===============================================================================

keep hhid mnf_val* mnf_kval* itc drop_for_comparability mnf_code
save `201213_monthly_nf', replace

collapse (sum) mnf_val* mnf_kval*, by(hhid)
sort hhid
save `201213_hh_nfood', replace

* add variables from household roster and food
*-------------------------------------------------------------------------------
use `201213_hh_mastercon', clear

merge 1:1 hhid using `201213_hh_nfood'
assert _m == 3
drop _m
...
keep hhid province district sector psu* mfood mfuel mcloth mndur mserv mperson mtrans mentert mdurab mboard mtfamily msaving mmisc
...
* LC the following lines added to keep empty variables for the following, since 
* we do not have boarders and servant file
*-------------------------------------------------------------------------------
#delimit ;
local var " mfood mfuel mcloth mndur mserv mperson mtrans mentert mdurab mboard 
			mtfamily msaving mmisc sb_food sb_nfood sb_exp " ;
#delimit cr
foreach v of local var{
gen `v'=0
}
*-------------------------------------------------------------------------------
save `201213_sbexp', replace
...
sort hhid
save `201213_sbexp_hh', replace

* add variables from household roster, food and nonfood
*-------------------------------------------------------------------------------
use `201213_hh_mastercon', clear
merge 1:1 hhid using `201213_sbexp_hh'
drop if _m==2
drop _m
...
use `201213_food', clear
merge m:1 hhid using `201213_hh_pcexp'
assert _m == 3
drop _m
...
merge 1:1 district using "${out}/hies201213_dist_cpi"
keep if _m==3
drop _m
sort district
save "${out}/hies201213_dist_cpi_all", replace
...
merge m:1 district using "${out}/hies201213_dist_cpi_all"
keep if _m==3
drop _m
save "${out}/hies201213_dist_cpi_all", replace
...
replace mnf_val_drop = (mnf_val_drop / cpi_dcs)/ hhsize
...
save `201213_f_major', replace

* NON FOOD
*-------------------------------------------------------------------------------
use `201213_monthly_nf', clear
rename  mnf_code mc
...
save `201213_nf_major', replace

* Merge file with major categories with consumption aggregate file
*-------------------------------------------------------------------------------
use "${out}\wfile201213", clear
merge 1:1 hhid using `201213_f_major'
assert _m == 3
drop _m
merge 1:1 hhid using `201213_nf_major'
assert _m == 3
drop _m
...
forvalues v=1(1)16{
gen rpc_nfval`v' = nfval`v' / cpi_dcs / hhsize
}
