/*------------------------------------------------------------------------------
					GMD Harmonization
--------------------------------------------------------------------------------
<_Program name_>   LKA_2019_HIES_v01_M_v01_A_SARMD_GEO.do	</_Program name_>
...
<_Version Control_>
Date:	06-26-2022
File:	LKA_2019_HIES_v01_M_v01_A_SARMD_GEO.do
- First version
</_Version Control_>
------------------------------------------------------------------------------*/
...
if ("`c(username)'"=="sunquat") {
	...
	* HH expenditure and income - for hhid
	merge 1:m psu using "HH_expenditure_hh_Income", nogen assert(match)
}
* global paths on WB computer
	...
	* merge in durable goods data
	...
	merge m:1 psu using `hh_level_data', nogen update 
	*assert(match)
	*</_Datalibweb request_>
}
