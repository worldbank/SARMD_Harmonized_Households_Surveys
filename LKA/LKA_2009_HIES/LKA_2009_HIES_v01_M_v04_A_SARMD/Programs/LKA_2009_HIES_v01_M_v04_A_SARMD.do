...
** DIRECTORY
	...
	*/
	...
	glo input     "$rootdatalibweb\SAR_DATABANK\LKA\LKA_2009_HIES\LKA_2009_HIES_v01_M"
	glo output    "$rootdatalibweb\SAR_DATABANK\LKA\LKA_2009_HIES\LKA_2009_HIES_v01_M_v04_A_SARMD"
...
** LOG FILE
	log using "$output\Doc\Technical\LKA_2009_HIES_v01_M_v04_A_SARMD.log",replace
...
*<_cpi_>
	...
	*keepusing(countrycode year urb syear cpi`year'_w ppp`year')
	...
	use "$pricedata", clear 
	tempfile pricedata 
	keep if countrycode=="LKA" & year==2009
	local year=2011
...
** KEEP VARIABLES - ALL
	...
	int_year  gaul_adm1_code gaul_adm2_code subnatid1 subnatid2 subnatid3 gaul_adm1_code 			  ///
	gaul_adm2_code ownhouse landholding tenure water_orig piped_water water_jmp sar_improved_water    ///
	electricity toilet_orig sewage_toilet toilet_jmp sar_improved_toilet  landphone cellphone computer ///
	internet hsize relationharm relationcs male age soc marital ed_mod_age everattend atschool        ///
	electricity literacy educy educat4 educat5 educat7 lb_mod_age lstatus lstatus_year empstat        ///
	empstat_year njobs njobs_year ocusec nlfreason unempldur_l unempldur_u industry_orig industry     ///
	occup_orig occup firmsize_l firmsize_u whours wage unitwage empstat_2 empstat_2_year industry_2   ///
	industry_orig_2 occup_2 wage_2 unitwage_2 contract healthins socialsec union rbirth_juris rbirth  ///
	rprevious_juris rprevious yrmove landphone cellphone computer radio television fan sewingmachine  ///
	washingmachine refrigerator lamp bicycle motorcycle motorcar cow buffalo chicken pline_nat        ///
	pline_int poor_nat poor_int spdef cpi ppp cpiperiod welfare welfshprosperity welfarenom welfaredef ///
	welfarenat food_share nfood_share quintile_cons_aggregate decile_cons_aggregate welfareother      ///
	welfaretype welfareothertype hhid
...
** DELETE MISSING VARIABLES
	...
	foreach var of varlist urban - welfareothertype {
	qui sum `var'
	scalar sclrc = r(mean)
	if sclrc==. {
	...
	keep countrycode year survey idh idp wgt pop_wgt strata psu vermast veralt subnatid1 subnatid2 welfareother hhid `keep' *type
	compress

	saveold "$output\Data\Harmonized\LKA_2009_HIES_v01_M_v04_A_SARMD_ALL.dta", replace version(12)
	*saveold "D:\SOUTH ASIA MICRO DATABASE\SAR_DATABANK\__REGIONAL\Individual Files\LKA_2009_HIES_v01_M_v04_A_SARMD-FULL_IND.dta", replace version(12)
...
note _dta: "LKA 2009" Variables NAMED with "v2" are those not compatible with latest round (2012). ///
 ...
 saveold "$output\Data\Harmonized\LKA_2009_HIES_v01_M_v04_A_SARMD_IND.dta", replace version(12)
 *saveold "D:\SOUTH ASIA MICRO DATABASE\SAR_DATABANK\__REGIONAL\Individual Files\LKA_2009_HIES_v01_M_v04_A_SARMD_IND.dta", replace version(12)
