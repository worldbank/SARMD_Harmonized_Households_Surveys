...
*-------------------------------------------------------------------------------
tempfile 2009_sec_1_all_demographic 2009_sec_1_demographic 2009_hh_master 2009_food ///
	     2009_food_hh 2009_hh_mastercon 2009_monthly_nf 2009_hh_nfood 2009_sbexp 2009_sbexp_hh ///
	     2009_hh_pcexp 2009_nf_major ashare0 pricefdb0 pricep0 priced0 pricen0 priceshare0 2009_f_major

*===============================================================================
* Slected variables from the HOUSEHOLD ROSTER
*===============================================================================
use "${data}/hies_2009_sec1_demo", clear
...
save `2009_sec_1_all_demographic', replace

* drop members living outside the household
*-------------------------------------------------------------------------------
drop if pno >=40

collapse (first) province district sector psu month weight (count) pno, by(hhid)
...
rename pno hhsize
save `2009_hh_master', replace

*===============================================================================
* FOOD Expenditure
*===============================================================================

use "${data}/hies_2009_sec4_1_food", clear
...
keep  hhid province district mfval mfquant mfkind* mlt_val mlt_qty mlt_kind itc mf_code 
sort hhid

save `2009_food', replace

collapse (sum) mfval mfkind* mlt_val mlt_kind, by(hhid)
sort hhid
save `2009_food_hh', replace
...
*-------------------------------------------------------------------------------
use `2009_hh_master', clear
merge 1:1 hhid using `2009_food_hh'
assert _m == 3
drop _m
save `2009_hh_mastercon', replace


*===============================================================================
* create file with NONFOOD expenditures
*===============================================================================
use "${data}/hies_2009_sec4_2_nonfood", clear
...
keep hhid mnf_val* mnf_kval* itc mnf_code
save `2009_monthly_nf', replace

collapse (sum) mnf_val* mnf_kval*, by(hhid)
sort hhid
save `2009_hh_nfood', replace

* add variables from household roster and food
*-------------------------------------------------------------------------------
use `2009_hh_mastercon', clear
merge 1:1 hhid using `2009_hh_nfood'
...
* SERVANT BOARDERS
*===============================================================================
use "${data}/hies_2009_sec4_3_1_serv_board.dta", clear
...
sort hhid
save `2009_sbexp', replace

collapse (sum) mfood mfuel mcloth mndur mserv mperson  ///
 mtrans mentert mdurab mboard mtfamily msaving mmisc sb_food sb_nfood sb_exp, by(hhid)
sort hhid
save `2009_sbexp_hh', replace

* add variables from household roster, food and nonfood
*-------------------------------------------------------------------------------
use `2009_hh_mastercon', clear
*merge 1:1 hhid using `2009_sbexp_hh'
merge hhid using `2009_sbexp_hh',sort
...
use `2009_hh_pcexp', clear
sort hhid
save `2009_hh_pcexp', replace

use `2009_food', clear
sort hhid
merge hhid using `2009_hh_pcexp'
tab _m
keep if _m==3
drop _m
...
drop qpd qpn
merge 1:1 district using "${out}/hies_2009_10_district_cpi"
rename cpi cpi_dcs
keep if _m==3
drop _m
sort district
save "${out}/hies2009_dist_cpi_all", replace
...
/* variable share is the share in selected items and ashare is share in all items*/
...
sort district
merge district using "${out}/hies2009_dist_cpi_all"
keep if _m==3
drop _m
save "${out}/hies2009_dist_cpi_all", replace
...
merge m:1 district using "${out}/hies2009_dist_cpi_all" 
assert _m == 3
drop _m
...
save `2009_f_major', replace


* NON FOOD
*-------------------------------------------------------------------------------
use `2009_monthly_nf', clear
rename  mnf_code mc
rename mnf_val val
collapse (sum) val, by(hhid mc)
sort hhid
reshape wide val, i( hhid ) j( mc )
mvencode val1- val15, mv(0) override
rename val* nfval*
save `2009_nf_major', replace

* Merge file with major categories with consumption aggregate file
*-------------------------------------------------------------------------------
use "${out}\wfile2009", clear
merge 1:1 hhid using `2009_f_major'
assert _m == 3
drop _m
merge 1:1 hhid using `2009_nf_major'
assert _m == 3
drop _m
