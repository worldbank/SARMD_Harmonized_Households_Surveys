...
<_Version Control_>
Date:	05-03-2020
File:	MDV_2019_HIES_v01_M_v01_A_SARMD_IND.do
- First version
</_Version Control_>
------------------------------------------------------------------------------*/
...
*------------------------------------------------------------------------------*/
	...
	*--------------------------------------------------------------------------*
	*#delimit cr
	datalibweb, country(`code') year(`year') type(SARMD) mod(GMD) verm(`vm') vera(`va') 
	*localpath(${rootdatalib}) local
	tempfile SARMD_GMD
	save `SARMD_GMD', replace
	...
	*#delimit ;
	...
	merge 1:1 hhid pid using `SARMD_GMD' 
...
replace sar_improved_water=1 if inlist(hh_drkwater,1,2,3,5,6,7)
replace sar_improved_water=0 if inlist(hh_drkwater,4)
*</_sar_improved_water_>
...
local vars_notcreated "industry_year industry_2_year subnatid1_sar subnatid2_sar subnatid3_sar subnatid4_sar"
foreach v of local vars_notcreated {
	gen `v'=. 
}
