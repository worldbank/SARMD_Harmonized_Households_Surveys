...
** Input databases 
	datalibweb, country(PAK) year(2004) type(GMD) mod(all) clear
	drop educat* welfare* welfshprosperity cpi* icp* ppp* veralt subnatid* 
	/*
	if _rc {
	...
	*/
	...
	preserve
	use "`output'\Data\Harmonized\PAK_2004_HIES_v01_M_v0`v'_A_SARMD_IND.dta", clear 
	rename idh hhid
	rename idp pid 
	...
	save `input_base'
	restore 
	merge 1:1 countrycode year hhid pid psu using `input_base'
	
	order countrycode code year- psu vermast veralt
...
*</_poor_int_>
	...
	saveold "`output'\Data\Harmonized\PAK_2004_HIES_v01_M_v0`v'_A_SARMD_GMD.dta", replace 
