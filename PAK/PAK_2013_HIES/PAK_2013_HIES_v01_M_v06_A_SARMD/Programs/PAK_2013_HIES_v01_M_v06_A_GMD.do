...
** Input databases 
	datalibweb, country(PAK) year(2013) type(GMD) mod(all) clear
	drop educat* welfare* welfshprosperity cpi* icp* psu urban subnatid*
	/*
	if _rc {
	...
	*/
	...
	preserve
	use "`output'\Data\Harmonized\PAK_2013_HIES_v01_M_v0`v'_A_SARMD_IND.dta", clear 
	rename idh hhid
	rename idp pid 
	keep countrycode code year hhid pid psu educat* welfare* welfshprosperity cpi* ppp*       ///
	pline_nat pop_wgt vermast veralt water_source water_original watertype_quest sanitation_source    ///
	sanitation_original toilet_acc pipedwater_acc improved* subnatid* gaul_adm*_code urban lb_mod_age
	tempfile input_base
	save `input_base'
	restore 
	merge 1:1 countrycode year hhid pid using `input_base'
	
	order countrycode code year hhid pid vermast veralt
	rename survname survey 
	rename lb_mod_age minlaborage
...
*<_poor_int_>
	...
	tab poor_int_19 [aw= weight_p]  if !mi(poor_int_19)
*</_poor_int_>
...
*<_poor_int_>
	...
	tab poor_int_32 [aw= weight_p]  if !mi(poor_int_32)
*</_poor_int_>
...
*<_poor_int_>
	...
	tab poor_int_55 [aw= weight_p]  if !mi(poor_int_55)
*</_poor_int_>

	saveold "`output'\Data\Harmonized\PAK_2013_HIES_v01_M_v0`v'_A_SARMD_GMD.dta", replace 
