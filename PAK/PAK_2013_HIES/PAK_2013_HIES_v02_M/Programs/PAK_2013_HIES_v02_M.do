/*------------------------------------------------------------------------------
  GMD Harmonization
--------------------------------------------------------------------------------
<_Program name_>   		PAK_2013_HIES_v02_M.do				   </_Program name_>
...
merge 1:1 hhcode idc using "${input}/sec_1b_.dta", nogen keep(master match) 
...
merge m:1 hhcode using "${input}/sec_5m.dta", nogen keep(master match)
...
keep if inlist(itc,801,802,804,805,8061,8062,810,811,812,813,814,815,816,817,818)
...
keep if codes==165
keep hhcode s10c3 s10c5 s10c6
collapse (sum) s10c3 s10c5 s10c6, by(hhcode)  
...
keep if codes==195
keep hhcode s10c3  
rename s10c3 s10c_inputsanimals
tempfile livestock3
save   `livestock3'
restore
...
* DEFLACTORS 	
merge m:1 psu using "${input}/psu_paasche_2013_pdef1.dta", nogen assert(match)
