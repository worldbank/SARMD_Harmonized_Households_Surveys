...
*<_Raw data_>
tempfile raw1
cap datalibweb, country(`code') year(`year') type(SARRAW) surveyid(`code'_`year'_PSLM_v01_M) filename(sec_6a.dta) 
if _rc!=0 {
	use "$rootdatalib\\`code'\\`code'_`year'_`survey'\\`code'_`year'_`survey'_v`vm'_M\Data\Stata\sec_6a.dta", clear 
}
...
*#delimit cr
cap datalibweb, country(`code') year(`year') type(`type') survey(`survey') vermast(`vm') veralt(`va') mod(IND) clear localpath(${rootdatalib}) local
if _rc!=0 {
...
egen dwelmat_exp= rowtotal(v143101-v143103), missing
*</_dwelmat_exp_>
...
egen dwelsvc_exp= rowtotal(v143201-v143202), missing
*</_dwelsvc_exp_>
...
gen garbage_exp= 12*(s5aq29a+s5aq29b)
...
notes gasoline_exp: Using the gasoline expense that is provided together with Petrol/ Diesel/ Mobil oil Expenses. Multiplied by 12 to annualize
*</_gasoline_exp _>
...
gen heatsource=.
replace heatsource=2 if s5aq09==7
replace heatsource=3 if s5aq09==8
replace heatsource=4 if s5aq09==2
replace heatsource=5 if s5aq09==4  | s5aq09==3 | s5aq09==5
replace heatsource=9 if s5aq09==11 | s5aq09==6 | s5aq09==9
replace heatsource=10 if s5aq09==10 | s5aq09==1
*</_heatsource_>
...
egen othhousing_exp=rowtotal(v156203-v156204), missing
*</_othhousing_exp_>
...
gen sewage_exp=v144201*12
notes sewage_exp: Multiplied by 12 to annualize
*</_sewage_exp_>
...
egen transfuel_exp=rowtotal(v172201-v172202), missing
*</_transfuel_exp_>
...
gen w_30m = inlist(s5aq14,1,2) if !missing(s5aq14)
*</_w_30m_>
...
gen w_avail=s5aq17
replace w_avail=0 if w_avail==2
replace w_avail=. if w_avail==3
*</_w_avail_>
...
gen waste=.
replace waste = 1 if s5aq28a == 1
replace waste = 3 if s5aq28a == 2
replace waste = 4 if s5aq28a == 3  & s5aq28b == 1
replace waste = 5 if s5aq28a == 3  & s5aq28b == 2
replace waste = 10 if s5aq28a == 3 & s5aq28b == 3
