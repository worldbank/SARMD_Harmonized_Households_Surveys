...
** DIRECTORY	
	global input   		 "${rootdatalib}\PAK\PAK_2018_HIES\PAK_2018_HIES_v01_M"
	global output  		 "${rootdatalib}\PAK\PAK_2018_HIES\PAK_2018_HIES_v01_M_v`va'_A_SARMD"
...
** LOG FILE
	cap log using "`output'\Doc\Technical\PAK_2018_`survey'_v`vm'_M_v`va'_A_SARMD.log",replace
...
*/
	...
	*save `aux', replace
	save "${output}\aux_DELETE.dta", replace 
...
*/
	...
	*merge 1:1 hhcode idc using `aux'
	merge 1:1 hhcode idc using "${output}\aux_DELETE.dta"
	tab _merge
...
*/
	...
	*merge 1:m hhcode  using `aux'
	merge 1:m hhcode using "${output}\aux_DELETE.dta"
	tab _merge
...
*/
	...
	*save `aux', replace
	...
	use "${input}\Data\Stata\CA_1819_sharing_v2.dta", clear
	tempfile comp
	save  `comp' , replace

	use "${output}\aux_DELETE.dta", clear
...
*/
	...
	use "${output}\aux_DELETE.dta", replace
...
**Add deflactor information	
	merge m:1 psu using "$rootdatalib\PAK\PAK_2018_HIES\PAK_2018_HIES_v01_M\Data\Stata\psu_paasche_2018_pdef1.dta", force 
...
**Add corrected welfare information	
	...
	merge 1:1 psu idh idp using "$rootdatalib\PAK\PAK_2018_HIES\PAK_2018_HIES_v01_M\Data\Stata\welfarenom_v01M_new.dta", force nogen
...
*<_educat7_>
	...
	*Currently attending
	...
	replace educat7 = 8 if (s2bq14 == 28 ) & s2bq01==3    & educat7==.
...
*****************************************************************************************************/
 ...
 saveold "${output}\Data\Harmonized\PAK_2018_HIES_v`vm'_M_v`va'_A_SARMD_ALL.dta", replace version(12)
...
** KEEP VARIABLES - ALL
	*do "$fixlabels\fixlabels", nostop
	...
	keep countrycode code year survey idh idp wgt pop_wgt strata psu vermast veralt urban int_month int_year fieldwork  ///
		subnatid1 subnatid2 subnatid3 gaul_adm*_code ownhouse landholding tenure water_original piped_water water_jmp sar_improved_water  ///
		...
		water_original water_source improved_water pipedwater_acc watertype_quest sanitation_original sanitation_source improved_sanitation toilet_acc ///
	     computer internet cooksource hsize relationharm relationcs male age soc marital ed_mod_age everattend ///
		 ...
		 landphone cellphone computer radio television fan sewingmachine washingmachine refrigerator lamp bicycle motorcycle motorcar ///
		 cow buffalo chicken  ///
		 ...
		 welfareother welfaretype welfareothertype welfareother welfaretype welfareothertype  
...
** DELETE MISSING VARIABLES
	...
	saveold "${output}\Data\Harmonized\PAK_2018_HIES_v`vm'_M_v`va'_A_SARMD_IND.dta", replace version(12)
	*saveold "${drive}\SOUTH ASIA MICRO DATABASE\SAR_DATABANK\__REGIONAL\Individual Files\PAK_2018_HIES_v01_M_v`va'_A_SARMD_IND.dta", replace version(13)

	
	*-------
	erase "${output}\aux_DELETE.dta"
