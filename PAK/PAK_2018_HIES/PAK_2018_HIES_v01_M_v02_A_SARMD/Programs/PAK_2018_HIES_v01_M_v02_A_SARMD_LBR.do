...
*<_Raw data_>
tempfile raw
cap datalibweb, country(`code') year(`year') type(SARRAW) surveyid(`code'_`year'_HIES_v01_M) filename(sec_1b (2).dta) localpath(${rootdatalib}) local
if _rc!=0 {
	use "$rootdatalib\\`code'\\`code'_`year'_`survey'\\`code'_`year'_`survey'_v`vm'_M\Data\Stata\sec_1b (2).dta", clear 
}
