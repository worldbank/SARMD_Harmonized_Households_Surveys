/*------------------------------------------------------------------------------
  GMD Harmonization
--------------------------------------------------------------------------------
<_Program name_>   		PAK_2015_HIES_v02_M.do				   </_Program name_>
...
merge m:1 hhcode using "${input}/sec_3a.dta", nogen keep(master match)

* HOUSEHOLD EXPENDITURE (sec_4abcde)
preserve 
use "${input}/sec_4abcde.dta", clear
keep if itc>=42101 & itc<=42103 
...
preserve 
use "${input}/poverty_1516.dta", clear 
keep if year==2015
tempfile consumption 
save `consumption'
restore 
merge m:1 hhcode using `consumption', nogen keepusing(nomexpend eqadultM peaexpM psupind npline texpend hhsizeM date) keep(master match)

* TRANSFERS RECEIVED AND PAID
preserve
use "${input}/sec_5a", clear
keep if inlist(itc,801,802,804,805,8061,8062,810,811,812,813,814,815,816,817,818,819)
...
keep hhcode code s6aq01 s6aq04
drop if code==910
tostring code, replace 
egen    code2 = concat(code)
replace code2 = strtoname(code2)
keep hhcode code2 s6aq01 s6aq04
reshape wide s6aq01 s6aq04, i(hhcode) j(code2) string
...
preserve 
use "${input}/sec_7a.dta", clear
keep if codes==105 | codes==107
keep hhcode codes s7ac1
rename s7ac1 s10c1
...
use "${input}/sec_7a.dta", clear
keep if codes==135 
keep hhcode s7ac3 s7ac4 s7ac5
rename s7ac3 s10c_production
rename s7ac4 s10c_landlord
tempfile agrisheet2
save   `agrisheet2'
restore
...
preserve
use "${input}/sec_7b.dta", clear
keep if codes==165
keep hhcode s7bc3 s7bc5 s7bc6
collapse (sum) s7bc3 s7bc5 s7bc6, by(hhcode)  
rename s7bc3 s10c_soldanimals
rename s7bc5 s10c_purchasedanimals
rename s7bc6 s10c_lostanimals
tempfile livestock1
save   `livestock1'
restore
...
keep if codes==195
keep hhcode s7bc3  
rename s7bc3 s10c_inputsanimals
tempfile livestock3
save   `livestock3'
restore
...
preserve
use "${input}/sec_8ab.dta", clear
keep if (codes==288 | codes==289) & s8a>0
keep hhcode idc s8a
rename s8a s11a_equipment
...
* DEFLACTORS 	
merge m:1 psu using "${input}/psu_paasche_2015_pdef1.dta", nogen assert(match)
