...
** DIRECTORY
	...
	local input   		 "${rootdatalib}\PAK\PAK_2015_HIES\PAK_2015_HIES_v01_M"
	local output  		 "${rootdatalib}\PAK\PAK_2015_HIES\PAK_2015_HIES_v01_M_v`va'_A_SARMD"
...
** Create folder
	cap mkdir "`output'"
	cap mkdir "`output'\Doc"
	cap mkdir "`output'\Programs"
	cap mkdir "`output'\Doc\Technical"
	cap mkdir "`output'\Data"
	cap mkdir "`output'\Data\Harmonized"

** LOG FILE
	log using "`output'\Doc\Technical\PAK_2015_`survey'_v`vm'_M_v`va'_A_SARMD.log",replace
...
**Add deflactor information	
	merge m:1 psu using "$rootdatalib\PAK\PAK_2015_HIES\PAK_2015_HIES_v01_M\Data\Stata\psu_paasche_2015_pdef1.dta", force 
...
/*
	...
	*ren cpi`year'_w cpi
	label variable cpi2011_06 "CPI (CPI from January 2016 base 2011)"
	label variable cpi2017_06 "CPI (CPI from January 2016 base 2017)"
*</_cpi_>
...
saveold "`output'\Data\Harmonized\PAK_2015_`survey'_v`vm'_M_v`va'_A_SARMD_ALL.dta", replace version(12)
	...
	keep countrycode code year survey idh idp wgt pop_wgt strata psu vermast veralt urban int_month int_year fieldwork  ///
		...
		water_original water_source improved_water pipedwater_acc watertype_quest sanitation_original sanitation_source improved_sanitation toilet_acc ///
	     ...
	     ocusec nlfreason unempldur_l unempldur_u industry_orig industry occup_orig occup firmsize_l firmsize_u whours wage ///
		  ...
		  union rbirth_juris rbirth rprevious_juris rprevious yrmove ///
		 ...
		 pline_nat pline_int* poor_nat poor_int* spdef cpi* ppp* welfare welfshprosperity welfarenom welfaredef eqadult welfarenat ///
...
** DELETE MISSING VARIABLES
	...
	saveold "`output'\Data\Harmonized\PAK_2015_`survey'_v`vm'_M_v`va'_A_SARMD_IND.dta", replace version(12)
	*saveold "${drive}\SOUTH ASIA MICRO DATABASE\SAR_DATABANK\__REGIONAL\Individual Files\PAK_2015_`survey'_v01_M_v02_A_SARMD_IND.dta", replace version(13)
