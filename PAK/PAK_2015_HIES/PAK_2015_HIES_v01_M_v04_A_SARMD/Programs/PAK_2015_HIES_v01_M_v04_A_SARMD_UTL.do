/*------------------------------------------------------------------------------
					GMD Harmonization
--------------------------------------------------------------------------------
<_Program name_>   PAK_2015_PSLM_v01_M_v04_A_GMD_UTL.do	</_Program name_>
...
<_Version Control_>
Date:	03-03-2019
File:	PAK_2015_PSLM_v01_M_v04_A_GMD_UTL.do
- First version
</_Version Control_>
------------------------------------------------------------------------------*/
...
local filename     "`code'_`year'_`survey'_v`vm'_M_v`va'_A_SARMD_UTL"
*</_Program setup_>;


*<_Raw data_>;
tempfile raw1
cap datalibweb, country(`code') year(`year') type(SARRAW) surveyid(`code'_`year'_PSLM_v01_M) filename(sec_4abcde.dta) 
if _rc!=0 {
	use "$rootdatalib\\`code'\\`code'_`year'_`survey'\\`code'_`year'_`survey'_v`vm'_M\Data\Stata\sec_4abcde.dta", clear 
}
keep if inlist(itc,45101,45102,45201,45202,45301,45401,45402,45403,45404,45405,45406,43101,43102,43103,43104,43105,43106,43107,43108,43109,43201,43202,43203,43204,43205,43206,43207,44101,44201,53301,56203,56204,83001,83002,83003,83004,83005,72201,72202,72203,72204,93101)

keep hhcode itc v1
reshape wide v1, i(hhcode) j(itc)
save `raw1'

tempfile raw2
cap datalibweb, country(`code') year(`year') type(SARRAW) surveyid(PAK_2015_PSLM_v01_M) filename(sec_3a.dta) 
if _rc!=0 {
	use "$rootdatalib\\`code'\\`code'_`year'_`survey'\\`code'_`year'_`survey'_v`vm'_M\Data\Stata\sec_3a.dta", clear 
}
...
gen coal_exp= 12*v145403;
notes coal_exp: Multiplied by 12 to annualize. Coal and peat collected together;
...
egen dwelmat_exp= rowtotal(v143101-v143109), missing;
*</_dwelmat_exp_>;
...
egen dwelsvc_exp= rowtotal(v143201-v143207), missing;
*</_dwelsvc_exp_>;
...
*gen electricity =s3aq05a;
*recode electricity (3=0) (1 2=1);
*</_electricity_>;
...
gen garbage_exp= 12*(s3aq19a+s3aq19b);
...
gen gas= s3aq05b;
recode gas (3=0) (1 2=1);
notes gas: Type of gas connection not collected in survey;
*</_gas_>;
...
gen kerosene_exp= 12*v145301;
notes kerosene_exp: Multiplied by 12 to annualize;
*</_kerosene_exp_>;
...
gen LPG_exp = 12*v145202;
notes LPG_exp: Multiplied by 12 to annualize;
*</_LPG_exp _>;
...
egen othhousing_exp= rowtotal(v153301 v156203 v156204);
*</_othhousing_exp_>;
...
egen othsol_exp= rowtotal(v145402 v145404 v145405 v145406), missing;
...
label define lbltoilet_orig 1 "1-Flush connected to public sewerage" 2 "2-Flush connected to pit" 3 "3-Flush connected to open drain" 4 "4-Dry raised latrine" 5 "5-Dry pit latrine" 6 "6-No toilet in the household", modify;
...
*clonevar sanitation_source=sanitation_source;
*</_sanitation_source_>;
...
egen transfuel_exp=rowtotal(v172201-v172204), missing;
*</_transfuel_exp_>;
...
gen w_30m = inlist(s3aq11,1,2) if !missing(s3aq11);
*</_w_30m_>;
...
gen waste= .;
replace waste = 1 if s3aq18a == 1;
replace waste = 3 if s3aq18a == 2;
replace waste = 4 if s3aq18a == 3 & s3aq18b == 1;
replace waste = 5 if s3aq18a == 3 & s3aq18b == 2;
replace waste = 10 if s3aq18a == 3 & s3aq18b == 3;
recode waste (1 3 = 4)
 (4 = 5)(2 = 8) (5 = 7) (7 = 10);
notes waste: Observations with no formal system at household or neighbourhood level were classified as 'other' (10) as there was no clarity on whether they burn, bury, compost or dispose in water;
*</_waste_>;
...
cap drop temp;
clonevar temp = s3aq06;
label define s3aq06 1 "1-Piped Water" 2 " 2-hand pump" 3 "3-motorized pumping/tubewell" 4 "4-open well" 5 "5-closed well" 6 "6-pound/canal/river/stream" 7 "7-spring" 8 "8-mineral water" 9 "9-tanker/truck/water bearer" 10 "10-filtration plant" 11 "11 others", modify;
...
gen water_exp= 12*v144101;
notes water_exp: Multiplied by 12 to annualize;
*</_water_exp_>;
...
gen gas_exp= 12*v145201;
notes gas_exp: Using the gas expense that is provided separately from LPG in original survey. Multiplied by 12 to annualize;
*</_gas_exp_>;
