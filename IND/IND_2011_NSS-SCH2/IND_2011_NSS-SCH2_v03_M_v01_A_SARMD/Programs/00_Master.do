...
global code         "IND"
global year         "2011"
global survey       "NSS-SCH2"
global vm           "03"
global va           "01"
global yearfolder   ="${code}"+"_"+"${year}"+"_"+"${survey}"

notes: possible answers for the following local: Y and N.
local includes_INCmodule "N" //Y if INC module was created for this version//
...
if "`includes_INCmodule'"=="N" {
	...
	if "${code}" == "IND" {
		foreach mod in  "IDN" "DEM" "LBR" "GEO" "UTL" "DWL" "CONS" "IND" { 
			glo module "`mod'"
...
if "`includes_INCmodule'"=="Y" {
	if ${year}<2016 {
		...
		if ${year}==2010 & "${code}" == "NPL" {
			set more on
			foreach mod in  "INC" "IND" "LBR" "IDN" "COR" "DEM" "GEO" "UTL" "DWL" "CONS"  { 
				glo module "`mod'"
				local filename     "${yearfolder}_v${vm}_M_v${va}_A_SARMD_`mod'"
				do "`dopath2'\\`filename'.do"
			}	
