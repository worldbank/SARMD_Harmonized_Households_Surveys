...
count 			if  fdq==. & b3q3==1 												// 321
gen aux = 1 	if  fdq==. & b3q3==1
...
set seed 12345
sort hhid pid
gen random = uniform() 	if  !inlist(b3q3,1,2) & fdq==1
...
replace fdq_new = 1 		if  fdq_new==. & b3q3==1 								// 321
...
replace fdq_new = . 		if  fdq_miss_head==1 & (n_fdq_new>n_fdq_or) & b3q3==2 	// 20

* Adjust welfare using new FDQ
for any ctry_adq fdtexp_own fdtexp_buy fdtexp nfdtexp totexp fdpindex nfdpindex pindex ctry_totexp hhsize: replace X = . if fdq_new==.

preserve 
keep hhid pid sector 
tempfile rural 
save `rural', replace
restore
...
merge 1:1 hhid pid using `rural', nogen
gen code = "IND"
...
gen 	poor_int_300 = ctry_totexp<pline_int_300
replace poor_int_300 = .				if  ctry_totexp==.
sum 	poor_int_300 [aw=wgt] 		if  !mi(poor_int_300)

gen pline_int_420 = 4.20*cpi2021*icp2021*365/12
gen 	poor_int_420 = ctry_totexp<pline_int_420 
replace poor_int_420 = .				if  ctry_totexp==.
sum 	poor_int_420 [aw=wgt] 		if  !mi(poor_int_420)

gen pline_int_830 = 8.30*cpi2021*icp2021*365/12
gen 	poor_int_830 = ctry_totexp<pline_int_830
replace poor_int_830 = .				if  ctry_totexp==.
