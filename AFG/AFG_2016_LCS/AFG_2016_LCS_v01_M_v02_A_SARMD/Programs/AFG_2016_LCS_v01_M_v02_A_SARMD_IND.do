...
*<_improved_water_>
	gen improved_water=.
	replace improved_water=1 if inlist(water_orig,1,2,3,4,5,7)
	replace improved_water=0 if inlist(water_orig,6,8,9,10,11) // Asuming other is not improved water source
...
egen electricity=rmin(q_4_14_a q_4_14_b)
...
/*
local variable_order "code year survey idh idp wgt pop_wgt strata psu vermast veralt urban int_year subnatid0 subnatid1 subnatid2 gaul_adm1_code hsize relationharm relationcs male age marital ed_mod_age atschool educy everattend literacy water_orig improved_water sanitation_orig improved_sanitation eye_dsablty hear_dsablty walk_dsablty conc_dsord slfcre_dsablty comm_dsablty landphone cellphone phone computer internet radio television tv_cable video refrigerator sewingmachine washingmachine stove ricecook fan ac etablet ewpump bicycle motorcycle oxcart boat motorcar canoe roof wall floor kitchen bath rooms areaspace ownhouse acqui_house acqui_land dwelownlti fem_dwelownlti dwelownti selldwel transdwel ownland doculand fem_doculand landownti sellland transland agriland area_agriland ownagriland area_ownagriland purch_agriland areapurch_agriland inher_agriland areainher_agriland rentout_agriland arearentout_agriland rentin_agriland arearentin_agriland docuagriland area_docuagriland fem_agrilandownti agrilandownti sellagriland transagriland typlivqrt dweltyp ybuilt pline_nat pline_natfood poor_nat poor_natfood welfarenat welfarenatfood educat4 educat5 educat7"
order `variable_order'

*keep `variable_order'

foreach var in buffalo chicken contract cow empstat_2 empstat_2_year healthins lamp lphone ocusec piped_water sar_improved_toilet  sar_improved_water   sewage_toilet ocialsec  toilet_jmp   typehouse  union   unitwage  unitwage_2   water_jmp firmsize_l firmsize_u food_share industry industry_2 industry_orig industry_orig_2 int_month lb_mod_age month nfood_share njobs pline_int poor_int quintile_cons_aggregate rbirth rbirth_juris  rprevious rprevious_juris socialsec spdef toilet_orig  unempldur_l unempldur_u wage wage_2 welfare welfaredef welfarenom welfareother welfareothertype welfaretype welfshprosperity whours yrmove {
	gen `var'=.a
}
gen subnatid3=""
*/
