...
if ("`c(username)'"=="sunquat") {
	...
	cd "${input}/Data/Stata"
	global pricedata "cpi_ppp_sarmd_weighted"
...
* global paths on WB computer
	...
	glo pricedata  "P:\SARMD\SARDATABANK\APPS\POVERTY_and_MPO\PovcalNet revisions\PovcalNet Spring 2016 revision\cpi_ppp_sarmd_weighted.dta"
	*<_Folder creation_>
...
*<_idp_>
	...
	clonevar idp_org = hhmemid
*</_idp_>

*<_pid_>
	egen pid=concat(hhid hhmemid), punct(-)
*</_pid_>
...
** HOUSE OWNERSHIP
*<_ownhouse_>
	recode q_2_9 (1/3 5=1) (4 8=3) (6/7=2) (9=4) (*=.), g(ownhouse)
*</_ownhouse_>

*<_typehouse_>
	recode q_2_9 (1/3 5=1) (4 8=3) (6/7=2) (9=4) (*=.), g(typehouse)
*</_typehouse_>
...
replace sar_improved_water=1 if inlist(water_orig,3,4,5,6,8,9,10,11,12,13,19,20)
replace sar_improved_water=0 if inlist(water_orig,1,2,7,14,15,16,17,18,21)
...
egen electricity=rmin(q_2_20_1 q_2_20_2 q_2_20_5 q_2_20_6)
recode electricity (2=0)
replace electricity=0 if mi(electricity) & !mi(electricity_all)
note electricity: electricity from public source electric grid, government generator, or community generator (engine or hydro).
...
*</_sar_improved_toilet_>
	...
	*<_toilet_acc_>
		recode q_2_26 (1/5 7=0) (6=3) (*=.), g(toilet_acc)
		#delimit 
...
gen agecat = .
note agecat: For AFG_2007_NRVA, age is continuous so did not create agecat.
*</_agecat_>
...
*<_atschool_>
	...
	recode atschool (2=0)
	* note: q_6_3 and q_6_8 differ for 4 individuals
	recode atschool (.=0) if q_6_3 == 2
...
*<_literacy_>
	...
	replace literacy = 1 if q_6_5_l >= 2 & q_6_5_l <.	//attended secondary school
	replace literacy = 1 if q_6_5_l==1 & inrange(q_6_5_y,2,7)	//those in years 2+ of primary school
...
*<_industrycat10_>
	recode q_9_18 (1=1) (2=2) (3/4=5) (5=3) (6=7) (7/8=6) (9/11=10) (12=9) (*=.) if lstatus==1, g(industrycat10)
*</_industrycat10_>
...
*<_landphone_>
	...
	recode q_5_9 (0=0) (1/max=1), g(landphone)
...
*<_lphone_>
	recode q_5_9 (0=0) (1/max=1), g(lphone)
*</_lphone_>
...
** COW
*<_cow_>
	gen cow = (q_3_2_a>0) if ~missing(q_3_2_a)
...
if ("`c(username)'"=="sunquat") {
	merge m:1 idh using "AFG_fnf_`y'", keepusing (food_share nfood_share quintile_cons_aggregate decile_cons_aggregate) nogen
}
* global paths on WB computer
else {
...
*<_cpi_>
	capture drop _merge
	gen urb=.
	merge m:1 countrycode year urb using "$pricedata", keepusing(countrycode urb year syear cpi`year_cpi'_w ppp`year_cpi')
	drop urb
	drop if _merge!=3
	drop _merge
...
*<_pline_int_>
	gen pline_int=2.15*cpi*ppp*365/12
...
*<_shared_toilet_>
g shared_toilet = (q_2_28==1) if inlist(q_2_28,1,2)
*<_shared_toilet_>
...
foreach var_notfound in industry_year industry_2_year industry_orig_year industry_orig_2_year occup_year ocusec_year subnatid4 subnatid2_sar subnatid3_sar subnatid4_sar {
	if strmatch("`var_notfound'","*_orig*") g `var_notfound' = ""
	else g int `var_notfound' = .
	note `var_notfound': AFG_2007_NRVA does not have any relevant questions or variables.
}

* create variables that do not have sufficient definitions from the SAR team
foreach var_notknown in converfactor gaul_adm2_code gaul_adm3_code sector welfshprtype {
	g `var_notknown'=.
	note `var_notknown': For AFG_2007_NRVA, I did not have a sufficient understanding of how `var_notfound' is defined from the SAR team, so it was created as missing.
}
