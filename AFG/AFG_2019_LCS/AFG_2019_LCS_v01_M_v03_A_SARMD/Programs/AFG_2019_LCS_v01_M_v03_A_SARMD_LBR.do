...
if ("`c(username)'"=="sunquat") {
	...
	* IND data
	...
	clonevar HH_ID=idh_org
	clonevar Mem_ID   =pid 
	* individual-level assebled data
	merge 1:m HH_ID Mem_ID using "labour_male", nogen assert(master match)
}
* global paths on WB computer
	...
	*use "${rootdatalib}\\`code'\\`code'_`year'_`survey'\\`code'_`year'_`survey'_v`vm'_M\Data\Stata\\`code'_`year'_`survey'_M.dta", clear
	...
	clonevar idp_org = Mem_ID
	tempfile labour_male
	save     `labour_male'	
	restore 
	merge 1:1 idh_org idp_org using `labour_male', gen(m_labour)
...
*clonevar unempldur_l = unempldur_l
...
*clonevar unempldur_u = unempldur_u
...
*clonevar industry_orig = industry_orig
*</_industry_orig_>
...
gen wage_total = q331_4 if lstatus==1 & empstat<=4
*</_wage_total_>
...
*clonevar industry_orig_2 = industry_orig_2
*</_industry_orig_2_>
...
clonevar industrycat10_2 = industry 
*</_industrycat10_2_>
...
*cap clonevar industry_orig_year=industry_orig_year
