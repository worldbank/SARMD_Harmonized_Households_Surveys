...
** DIRECTORY
	...
	*/
	...
	glo rootdatalibweb "P:\SARMD\SARDATABANK"
	glo input     "$rootdatalibweb\SAR_DATABANK\NPL\NPL_2010_LSS-III\NPL_2010_LSS-III_v01_M"
	glo output    "$rootdatalibweb\SAR_DATABANK\NPL\NPL_2010_LSS-III\NPL_2010_LSS-III_v01_M_v05_A_SARMD"
	glo pricedata "$rootdatalibweb\CPI\cpi_ppp_sarmd_weighted.dta"
	glo shares    "$rootdatalibweb\APPS\DATA CHECK\Food and non-food shares\NPL"
	glo fixlabels "$rootdatalibweb\APPS\DATA CHECK\Label fixing"
	
	
** LOG FILE
	log using "$output\Doc\Technical\NPL_2010_LSS-III_v01_M_v05_A_SARMD.log",replace
...
replace toilet_jmp=2 if toilet_orig==2
replace toilet_jmp=12 if toilet_orig==5
replace toilet_jmp=13 if inlist(toilet_orig,3,4)
label var toilet_jmp "Access to sanitation facility-using Joint Monitoring Program categories"
#delimit 
la def lbltoilet_jmpnew 1 "Flush to piped sewer  system"
					...
					13 "Other";
#delimit cr
la val toilet_jmp lbltoilet_jmpnew
*</_toilet_jmp_>
...
*</_sar_improved_toilet_>
	...
	*<_sanitation_source_>
		...
		replace sanitation_source=8 if v02_26==3
		replace sanitation_source=8 if v02_26==4
	...
	*<_improved_sanitation_>
		...
		replace improved_sanitation=1 if inlist(sanitation_source,1,2,3,4,5,8)
		replace improved_sanitation=0 if inlist(sanitation_source,6,7,9,10,11,12,13,14)
	*</_improved_sanitation_>
...
*<_open_def_>
	gen open_def=.
	replace open_def=0 if sanitation_source!=13 & sanitation_source!=. 
	replace open_def=1 if sanitation_source==13
	label var open_def "Whether a household has access to any sanitation facility"
*</_open_def_>
...
** AGECAT
*<_agecat_>
gen agecat="15 years or younger" if age<15
...
label var agecat "Age of individual (categorical)"
*</_age_>
...
** INDUSTRY CLASSIFICATION
*<_industrycat10_>
	gen byte industrycat10=industry
...
*<_industrycat4_>
	...
	label var industrycat4 "Sector/industry of employment (4 categories)"
...
*<_wmonths_>
	...
	label var wmonths "Months worked in the last 12 months"
*</_wmonths_>
...
*<_cpi_>
	...
	*keepusing(countrycode year urb syear cpi`year'_w ppp`year')
	...
	use "$pricedata", clear 
	tempfile pricedata 
	keep if countrycode=="NPL" & year==2010
	local year=2011
	keep countrycode year syear cpi`year'_w ppp`year'
	save `pricedata'
	restore
	merge m:1 countrycode year using `pricedata'
	*drop urb
	drop if _merge!=3
	drop _merge
...
** KEEP VARIABLES - ALL
	...
	keep countrycode year survey idh idp wgt pop_wgt strata psu vermast veralt urban int_month int_year fieldwork   ///
		subnatid1 subnatid2 subnatid3 subnatid4 gaul_adm1_code ownhouse landholding tenure water_orig piped_water   ///
		water_jmp sar_improved_water  electricity toilet_orig sewage_toilet toilet_jmp sar_improved_toilet landphone cellphone ///
		water_original water_source improved_water pipedwater_acc watertype_quest sanitation_original sanitation_source ///
		improved_sanitation toilet_acc computer internet hsize relationharm relationcs male age soc marital ed_mod_age  ///
		everattend atschool electricity literacy educy educat4 educat5 educat7 lb_mod_age lstatus lstatus_year empstat  ///
		empstat_year njobs njobs_year ocusec nlfreason unempldur_l unempldur_u industry_orig industry occup_orig occup  ///
		firmsize_l firmsize_u whours wage unitwage empstat_2 empstat_2_year industry_2 industry_orig_2 occup_2 wage_2   ///
		unitwage_2 contract healthins socialsec union rbirth_juris rbirth rprevious_juris rprevious yrmove landphone    ///
		cellphone computer radio television fan sewingmachine washingmachine refrigerator lamp bicycle motorcycle motorcar cow  ///
		buffalo chicken pline_nat pline_int poor_nat poor_int spdef cpi ppp cpiperiod welfare welfshprosperity          ///
		welfarenom food_share nfood_share quintile_cons_aggregate decile_cons_aggregate welfaredef welfarenat           ///
		welfareother welfaretype welfareothertype  harmonization agecat industrycat10  industrycat4 wmonths open_def    
...
** ORDER VARIABLES
	...
	order countrycode year survey idh idp wgt pop_wgt strata psu vermast veralt urban int_month int_year fieldwork  ///
		subnatid1 subnatid2 subnatid3 subnatid4 gaul_adm1_code ownhouse landholding tenure water_orig piped_water   ///
		water_jmp sar_improved_water water_original water_source improved_water pipedwater_acc watertype_quest      ///
		electricity toilet_orig sewage_toilet toilet_jmp sar_improved_toilet sanitation_original sanitation_source  ///
		improved_sanitation toilet_acc landphone cellphone computer internet hsize relationharm relationcs male age ///
		soc marital ed_mod_age everattend atschool electricity literacy educy educat4 educat5 educat7 lb_mod_age    ///
		lstatus lstatus_year empstat empstat_year njobs njobs_year ocusec nlfreason unempldur_l unempldur_u         ///
		industry_orig industry occup_orig occup firmsize_l firmsize_u whours wage unitwage empstat_2 empstat_2_year ///
		industry_2 industry_orig_2 occup_2 wage_2 unitwage_2 contract healthins socialsec union rbirth_juris rbirth ///
		rprevious_juris rprevious yrmove landphone cellphone computer radio television fan sewingmachine washingmachine ///
		refrigerator lamp bicycle motorcycle motorcar cow buffalo chicken pline_nat pline_int poor_nat poor_int spdef   /// 
		cpi ppp cpiperiod welfare welfshprosperity welfarenom food_share nfood_share quintile_cons_aggregate        ///
		decile_cons_aggregate welfaredef welfarenat welfareother welfaretype welfareothertype  harmonization agecat
...
** DELETE MISSING VARIABLES
	...
	*keep countrycode year survey idh idp wgt pop_wgt strata psu vermast veralt ${keep} *type
	...
	saveold "$output\Data\Harmonized\NPL_2010_LSS-III_v01_M_v05_A_SARMD_IND.dta", replace version(12)
	saveold "$rootdatalibweb\SAR_DATABANK\__REGIONAL\Individual Files\NPL_2010_LSS-III_v01_M_v05_A_SARMD_IND.dta", replace version(12)
	notes
