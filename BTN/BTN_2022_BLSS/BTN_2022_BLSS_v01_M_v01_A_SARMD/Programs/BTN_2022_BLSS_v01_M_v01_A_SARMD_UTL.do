...
if ("`c(username)'"=="sunquat") {
	...
	* load and merge data
	use "${rootdatalib}/BTN/BTN_2022_BLSS/BTN_2022_BLSS_v01_M/Data/Stata/BTN_2022_BLSS_v01_M", clear
	* merge in block 6: priorities, credits and opinions
	merge m:1 interview__id using "${rootdatalib}/BTN/BTN_2022_BLSS/BTN_2022_BLSS_v01_M/Data/Stata/block6_pro_cleaned", nogen assert(match)
}
* global paths on WB computer
	...
	* weights
	datalibweb, country(`code') year(`year') type(SARRAW) filename(block6_pro_cleaned.dta) local localpath(${rootdatalib})
	save `household_level_data'	, replace
	* merge in main data
