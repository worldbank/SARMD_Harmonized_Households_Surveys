...
** LOG FILE
	log using "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M_v`va'_A_SARMD\Doc\Technical\BTN_2003_BLSS_v`vm'_M_v`va'_A_SARMD.log",replace
...
** DATABASE ASSEMBLENT
	...
	* PREPARE DATASETS
	...
	use "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M\Data\Stata\hroster_edited.dta" 
	order stratum dzongkha town block houseno idno
	sort  stratum dzongkha town block houseno idno
	tempfile roster
	save `roster'
	
	use "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M\Data\Stata\block2_edited.dta" 
	order stratum dzongkha town block houseno idno
	sort  stratum dzongkha town block houseno idno
	tempfile individual
	save `individual'
	
	use "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M\Data\Stata\block1_edited.dta" 
	order stratum dzongkha town block houseno
	sort  stratum dzongkha town block houseno
	tempfile housing
	save `housing'
	
	use "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M\Data\Stata\block3_edited.dta" 
	order stratum dzongkha town block houseno
	sort  stratum dzongkha town block houseno
	tempfile assets
	save `assets'
	
	use "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M\Data\Stata\block7_edited.dta" 
	order stratum dzongkha town block houseno
	sort  stratum dzongkha town block houseno
	tempfile income
	save `income'
	
	use "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M\Data\Stata\paachse_index.dta" 
	order stratum dzongkha town block houseno
	sort  stratum dzongkha town block houseno
	tempfile weight
	save `weight'
	
	use "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M\Data\Stata\consumption_total.dta" 
	order stratum dzongkha town block houseno
	sort  stratum dzongkha town block houseno
...
*</_idh_>
	...
	merge m:1 idh using "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M\Data\Stata\pcc.dta" 
	drop _merge
...
** HOUSE OWNERSHIP
*<_ownhouse_>
	g		ownhouse = 1 if b1_q2==1
	replace	ownhouse = 2 if inlist(b1_q3,1,2)
	replace	ownhouse = 3 if b1_q3==3
	note ownhouse: For BTN_2003_BLSS, we assumed those who did not own their home (b1_q2 = 2) AND were not paying rent (b1_q3 = 3) were provided for free (ownhouse = 3). It could be that they are in the house without permission (ownhouse = 4), but we did not know and categorized all into "provided for free" based on our best guess.
*</_ownhouse_>
...
** ORIGINAL SANITATION CATEGORIES 
	...
	*</_toilet_acc_>
	...
	gen shared_toilet =.
	replace shared_toilet = 1 if b1_q17==1
	replace shared_toilet = 0 if b1_q17==2
