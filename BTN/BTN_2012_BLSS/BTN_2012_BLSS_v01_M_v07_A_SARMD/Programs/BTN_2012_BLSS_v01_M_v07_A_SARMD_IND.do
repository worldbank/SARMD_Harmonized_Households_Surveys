...
** DATABASE ASSEMBLENT
	...
	use "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M/Data/Stata\block1.dta", clear
	
	merge m:1 bchiwog hh6 using "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M/Data/Stata\block2.dta"
	drop _merge
	
	merge m:1 bchiwog hh6 using "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M/Data/Stata\block3.dta"
	drop _merge

	merge m:1 bchiwog hh6 using  "${rootdatalib}\\`code'\\`yearfolder'\\`yearfolder'_v`vm'_M/Data/Stata\poverty_estimate.dta"
	drop _merge
...
*<_ownhouse_>
	...
	note ownhouse: For BTN_2012_BLSS, we assumed those who did not own their home (hs2 = 2) AND were not paying rent (hs3 = 3) were provided for free (ownhouse = 3). It could be that they are in the house without permission (ownhouse = 4), but we did not know and categorized all into "provided for free" based on our best guess.
*</_ownhouse_>
...
** COMPUTER
*<_computer_>
	* note: variable already exists in harmonized form
